...
namespace Yohash.Tools
{
  public static class Interpolations
  {
    public static float InterpolateHighestNearestValuePreserving(this float[,] matrix, float x, float y)
    {
      ...
      var xc = (int)System.Math.Ceiling(x);
      var yc = (int)System.Math.Ceiling(y);
      var xf = xc - 1;
      var yf = yc - 1;
    ...
    public static float[,] Linear1stOrderSplat(float x, float y, float scalar = 1)
    {
      ...
      return new float[,] {
        { System.Math.Min(1 - dx, 1 - dy) * scalar,  System.Math.Min(1 - dx, dy) * scalar },
        { System.Math.Min(dx, 1 - dy) * scalar,      System.Math.Min(dx, dy) * scalar }
    };
    }
