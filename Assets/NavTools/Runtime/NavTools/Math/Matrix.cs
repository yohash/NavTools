...
namespace Yohash.Tools
{
  ...
  {
    private static double radiansToDegrees = 180 / System.Math.PI;
    private static double degressToRadians = System.Math.PI / 180;
    ...
    public static float[,] Rotate(this float[,] matrix, float degrees)
    {
      ...
      var na = System.Math.Round(System.Math.Abs(n * System.Math.Cos(radians)), precision);
      var nb = System.Math.Round(System.Math.Abs(m * System.Math.Sin(radians)), precision);
      var ma = System.Math.Round(System.Math.Abs(m * System.Math.Cos(radians)), precision);
      var mb = System.Math.Round(System.Math.Abs(n * System.Math.Sin(radians)), precision);

      // compute new dimensions of the matrix required to hold the rotated matrix
      var N = System.Math.Ceiling(na + nb);
      var M = System.Math.Ceiling(ma + mb);
      ...
      float invTransform(int u, int v)
      {
        ...
        var ur = ut * System.Math.Cos(radians) + vt * System.Math.Sin(radians);
        var vr = -ut * System.Math.Sin(radians) + vt * System.Math.Cos(radians);
        // (3) re-scale to original frame of reference
        var x = System.Math.Round(ur + (n - 1) / 2f, precision);
        var y = System.Math.Round(vr + (m - 1) / 2f, precision);
